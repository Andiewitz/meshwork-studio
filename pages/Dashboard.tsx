  ...
  const fetchFlows = async () => {
    if (user) {
      try {
        ...
        const hasSeeded = safeStorage.getItem('meshwork_guest_seeded');
        ...
        if (data.length === 0 && user.uid === 'dev-guest-123' && !hasSeeded) {
           ...
           safeStorage.setItem('meshwork_guest_seeded', 'true');
        } else {
           setFlows(data);
        }
      } catch (error) {
      ...
      <div className="p-6 md:p-8 max-w-6xl mx-auto space-y-8 pb-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-auto lg:h-[320px]">
            ...
            <div 
                ...
                className="bg-white rounded-3xl border-2 border-dashed border-slate-400 hover:border-emerald-500 hover:bg-emerald-50 transition-all duration-300 group flex flex-col items-center justify-center gap-6 cursor-pointer relative overflow-hidden"
            >
        ...
        {/* Material UI FAB verification */}
        <div className="fixed bottom-8 right-8 z-50">
          <Tooltip title="Create Quick Project" placement="left">
            <Fab 
              color="primary" 
              aria-label="add"
              onClick={() => navigate('/flow/new')}
